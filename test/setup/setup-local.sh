...
function get_environment {
  ...
  # Get the environment variables from the .github/workflows/ci.yml file with sed
  declare -a ci_yml
  ci_yml[0]="$(sed -n 's/.*- NGINX_CONTAINER_NAME=//p' "$LOCAL_BUILD_DIR/.github/workflows/ci.yml")"
  ci_yml[1]="$(sed -n 's/.*- DOCKER_GEN_CONTAINER_NAME=//p' "$LOCAL_BUILD_DIR/.github/workflows/ci.yml")"
  ci_yml[2]="$(sed -n 's/.*- TEST_DOMAINS=//p' "$LOCAL_BUILD_DIR/.github/workflows/ci.yml")"

  # If environment variable where sourced or manually set use them, else use those from 
  # .github/workflows/ci.yml
  export NGINX_CONTAINER_NAME="${NGINX_CONTAINER_NAME:-${ci_yml[0]}}"
  export DOCKER_GEN_CONTAINER_NAME="${DOCKER_GEN_CONTAINER_NAME:-${ci_yml[1]}}"
  export TEST_DOMAINS="${TEST_DOMAINS:-${ci_yml[2]}}"
